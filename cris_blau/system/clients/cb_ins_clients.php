<?php
    ...
    //Estas linhas são responsáveis por receber o nome, email, telefone, data de nascimento, rg, cpf, cep, cidade, bairro, logradouro, numero, complemento da fmins via POST.

    $msg_titulo = "Erro";
    //Esta linha é responsável por definir a variável msg_titulo, com o valor ERRO.

    $data = explode('/', $p_data_nasc);
    //Esta linha é responsável por explodir a variável p_data_nasc nas barras, que vem da fmins.

    if($p_nome==""){
        //Se a variável p_nome for vazia, chama a função mensagens.
        $mensagem = mensagens('Validação', 'nome');
    }else if(!valida_nome($p_nome, 45)){
            //Se a variável p_nome, possuir números, caracteres especiais ou passar de 45 caracteres, chama a função mensagens.
                                                                        ...
                                                                        $mensagem = mensagens('Validação', 'logradouro');
                                                                            ...
                                                                            $mensagem = mensagens('Validação Alfanumericos', 'logradouro');
                                                                                ...
                                                                                $mensagem = mensagens('Validação', 'numero');
                                                                                    ...
                                                                                    //Este bloco é responsável por fazer a seleção dos clientes que tenham o email igual a variável p_email, o rg igual a variável p_rg ou o cpf igual a variável p_cpf.
                                                                                    $sql_sel_clients = "SELECT * FROM clients WHERE email='".$p_email."' OR rg='".$p_rg."' OR cpf='".$p_cpf."'";
                                                                                    $sql_sel_clients_preparado = $conexaobd->prepare($sql_sel_clients);
                                                                                    $sql_sel_clients_preparado->execute();

                                                                                    //Este bloco é responsável por verificar se a contagem de registros com a condição acima é zero, então faz a inserção, se não, diz que o cliente já existe.
                                                                                    if($sql_sel_clients_preparado->rowCount()==0){
                                                                                        ...
                                                                                        $sql_ins_clients_resultado = adicionar($tabela, $dados);
                                                                                        //Chama a funçao adicionar e atribui o valor a variável sql_ins_clients_resultado.

                                                                                        //Este bloco é responsável por exibir se inserção funcionou ou não.
                                                                                        if($sql_ins_clients_resultado){
                                                                                            $msg_titulo = "Confirmação";
                                                                                            $mensagem = mensagens('Sucesso', 'Cliente', 'Cadastro');
                                                                                        }else{
                                                                                            $mensagem = mensagens('Erro bd', 'cliente', 'cadastrar');
                                                                                        }
                                                                                    }else{
...
<div class="message">
    ...
    <a href="?folder=clients/&file=cb_fmins_clients&ext=php" onClick="voltar()"><img src="../layout/images/back.png">Voltar</a>
</div>