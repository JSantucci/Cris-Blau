<?php
    ...
    $idreservado = array_keys($reservados);
    unset($idreservado[0]);
    for($i=1; $i<COUNT($reservados); $i++){
        ...
        if(isset($reservados[$sql_sel_kits_has_items_dados[$i]['itemsid']])) {
            if (($reservados[$sql_sel_kits_has_items_dados[$i]['itemsid']] == 0) || ($reservados[$sql_sel_kits_has_items_dados[$i]['itemsid']] - $sql_sel_kits_has_items_dados[$i]['itemquantity'] < 0)) {
                $resultado[$sql_sel_kits_has_items_dados[$i]['itemsid']] = $sql_sel_kits_has_items_dados[$i]['itemsid'];
            }
        }
    }

    $idresultado = array_keys($resultado);
    for($i=0; $i<COUNT($resultado); $i++){
        $sql_sel_kits = "SELECT kits.id AS kitsid, kits.name AS kitsname FROM kits INNER JOIN kits_has_items ON kits.id=kits_has_items.kits_id WHERE items_id='".$idresultado[$i]."'";
    ...
    for($i=0; $i<COUNT($sql_sel_kits_dados); $i++) {
        $resultados[$i] = $sql_sel_kits_dados[$i]['kitsid'];
    }
